...
create table if not exists public.hospitals (
  id uuid primary key default gen_random_uuid(), organization_id uuid, name text not null, legal_name text,
  registration_number text unique, address_line1 text, address_line2 text, city text, state text, postal_code text,
  country text not null default 'India', phone text, email text,
...
create table if not exists public.hospital_capabilities (
  ...
  specialty text, capability_type text not null check (capability_type in ('SPECIALTY','EQUIPMENT','SERVICE','FACILITY')),
  capability_name text not null, insurance_networks text[] not null default '{}',
...
create table if not exists public.specialists (
  ...
  full_name text not null, specialty text not null, sub_specialty text, license_number text unique, license_issuer text,
  license_expires_at date, credential_status text not null default 'PENDING' check (credential_status in ('PENDING','VERIFIED','EXPIRED','REJECTED')),
  review_queue_status text not null default 'AVAILABLE' check (review_queue_status in ('AVAILABLE','ASSIGNED','ON_HOLD','INACTIVE')),
  profile jsonb not null default '{}'::jsonb, created_at timestamptz not null default now(), updated_at timestamptz not null default now()
);
create table if not exists public.specialist_hospital_affiliations (
  ...
  is_primary boolean not null default false, created_at timestamptz not null default now(), primary key (specialist_id, hospital_id)
);
...
create table if not exists public.requests (
  id uuid primary key default gen_random_uuid(), request_id text unique not null, patient_id uuid not null references public.users(id) on delete restrict,
  request_type text, description text not null, request_source text not null default 'patient_portal',
  ...
  document_uploaded boolean not null default false, ai_classification jsonb not null default '{}'::jsonb, ai_confidence numeric(5,4), ai_reason text,
  created_at timestamptz not null default now(), updated_at timestamptz not null default now()
);
...
create table if not exists public.referrals (
  id uuid primary key default gen_random_uuid(), referral_id text unique not null,
  request_id uuid not null references public.requests(id) on delete restrict, hospital_id uuid not null references public.hospitals(id) on delete restrict,
  ...
  clinical_summary text, reason text, received_at timestamptz, reviewed_at timestamptz, decided_at timestamptz,
  created_at timestamptz not null default now(), updated_at timestamptz not null default now()
);
...
create table if not exists public.appointments (
  ...
  patient_id uuid not null references public.users(id) on delete restrict, hospital_id uuid not null references public.hospitals(id) on delete restrict,
  specialist_id uuid not null references public.specialists(id) on delete restrict, starts_at timestamptz not null, ends_at timestamptz not null,
  status text not null default 'AVAILABLE' check (status in ('AVAILABLE','HELD','BOOKED','CONFIRMED','CANCELLED','COMPLETED','NO_SHOW')),
  booking_reference text unique, metadata jsonb not null default '{}'::jsonb, created_at timestamptz not null default now(), updated_at timestamptz not null default now(),
...
create table if not exists public.documents (
  id uuid primary key default gen_random_uuid(), patient_id uuid not null references public.users(id) on delete restrict,
  request_id uuid references public.requests(id) on delete set null, referral_id uuid references public.referrals(id) on delete set null,
  storage_provider text not null default 'supabase', storage_bucket text not null, storage_path text not null, original_filename text not null,
  content_type text, file_size_bytes bigint, checksum_sha256 text,
  ...
  ocr_text text, validation_log jsonb not null default '[]'::jsonb, metadata jsonb not null default '{}'::jsonb,
  created_at timestamptz not null default now(), updated_at timestamptz not null default now()
);
...
create table if not exists public.audit_logs (
  ...
  request_id uuid references public.requests(id) on delete set null, previous_state jsonb, new_state jsonb,
...
create or replace function public.set_domain_updated_at() returns trigger language plpgsql as $$
...
-- Current application access uses the server-only service-role client. Browser RLS policies can be added later.
