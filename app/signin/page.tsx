...
const roles = [
  { href: "/patient/login", label: "Patient", description: "Sign in to your personal healthcare workspace.", icon: "P" },
  { href: "/hospital/login", label: "Hospital", description: "Sign in to your hospital operations workspace.", icon: "H" },
  { href: "/insurance/login", label: "Insurance", description: "Sign in to your insurance operations workspace.", icon: "I" },
  { href: "/admin/login", label: "Admin", description: "Sign in to MediSync administration.", icon: "A" },
];
  ...
  return (
    <main className="mesh-bg relative min-h-screen overflow-hidden px-5 py-8 text-[var(--ink)] md:px-8 md:py-10">
      ...
      <section className="relative z-10 mx-auto max-w-6xl py-12 md:py-16">
        <div className="mx-auto max-w-3xl text-center"><p className="mb-5 inline-flex rounded-full border border-black/5 bg-white/60 px-4 py-2 text-xs font-bold uppercase tracking-[.16em] text-[var(--care)] backdrop-blur">Welcome back</p><h1 className="font-display text-5xl font-extrabold leading-tight tracking-[-.045em] md:text-7xl">Choose your <span className="text-[var(--care)]">sign-in.</span></h1><p className="mx-auto mt-6 max-w-2xl text-base leading-7 text-[var(--muted-strong)] md:text-lg">Select your MediSync portal to continue securely.</p></div>
        <div className="mt-10 grid gap-4 sm:grid-cols-2">
          {roles.map((role, index) => <Link key={role.href} href={role.href} className={`glass reveal reveal-delay-${Math.min(index + 1, 3)} group block rounded-[28px] p-6 transition-all duration-300 hover:-translate-y-1 hover:shadow-[var(--shadow-md)] md:p-7`}>
            ...
            <h2 className="mt-7 font-display text-2xl font-extrabold">{role.label}</h2><p className="mt-2 text-sm leading-6 text-[var(--muted)]">{role.description}</p><p className="mt-6 text-xs font-bold uppercase tracking-[.14em] text-[var(--care)]">Sign in →</p>
          </Link>)}
        </div>
        <div className="mx-auto mt-8 max-w-3xl rounded-[24px] bg-white/60 p-5 text-center text-xs leading-5 text-[var(--muted-strong)] backdrop-blur">Forgot your password? <Link href="/forgot-password" className="font-bold text-[var(--care)] hover:underline">Get OTP by email →</Link></div>
      </section>
    </main>
  );
}
