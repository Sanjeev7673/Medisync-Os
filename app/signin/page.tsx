  ...
  return (
    <main className="relative min-h-screen overflow-hidden bg-[#090D16] text-white">
      ...
      <header className="relative z-10 mx-auto flex max-w-7xl items-center justify-between px-6 py-6">
        ...
        <button type="button" onClick={() => go("/signup")} className="rounded-full border border-white/10 bg-white/[.05] px-4 py-2.5 text-xs font-bold text-slate-300 hover:bg-white/10">Sign up</button>
      </header>
        ...
        <div className="w-full max-w-md rounded-[30px] border border-white/10 bg-white/[.045] p-2 shadow-[0_30px_100px_rgba(0,0,0,.5)] backdrop-blur-2xl">
          <div className="rounded-[24px] border border-white/[.07] bg-[#0D1320]/95 p-7 sm:p-9">
            ...
            <form onSubmit={submit} className="space-y-4">
              ...
              <label className="block"><div className="mb-2 flex justify-between"><span className="text-xs font-bold text-slate-300">Password</span><button type="button" onClick={() => go(`/forgot-password?role=${role}`)} className="text-xs font-bold text-cyan-300 hover:text-cyan-200">Forgot password?</button></div><div className="relative"><input required type={showPassword ? "text" : "password"} value={password} onChange={e => setPassword(e.target.value)} placeholder="Enter your password" className="w-full rounded-xl border border-white/10 bg-white/[.04] px-4 py-3.5 pr-12 text-sm text-white outline-none placeholder:text-slate-600 focus:border-emerald-400/50 focus:ring-2 focus:ring-emerald-400/10" /><button type="button" onClick={() => setShowPassword(v => !v)} className="absolute right-3 top-1/2 -translate-y-1/2 text-slate-500 hover:text-white" aria-label="Toggle password visibility">{showPassword ? <EyeOff className="h-4 w-4" /> : <Eye className="h-4 w-4" />}</button></div></label>
            ...
            <button type="button" onClick={() => go(`/forgot-password?role=${role}`)} className="block w-full rounded-xl border border-cyan-300/15 bg-cyan-300/[.04] px-4 py-3 text-center text-xs font-bold text-cyan-200 hover:bg-cyan-300/[.08]">Get OTP by email</button>
            <p className="mt-6 text-center text-xs text-slate-500">New to MediSync? <button type="button" onClick={() => go("/signup")} className="font-bold text-emerald-300">Sign up</button></p>
          </div>
        </div>
      </section>
    </main>
