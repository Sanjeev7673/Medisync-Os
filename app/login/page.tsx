      ...
      <section className="relative z-10 mx-auto max-w-6xl py-12 md:py-16">
        ...
        <div className="mt-10 grid gap-4 sm:grid-cols-2">
          {portals.map((portal, index) => (
            <button type="button" key={portal.href} onClick={() => openPortal(portal.href)} className={`glass reveal reveal-delay-${Math.min(index + 1, 3)} group block w-full cursor-pointer rounded-[28px] p-6 text-left transition-all duration-300 hover:-translate-y-1 hover:shadow-[var(--shadow-md)] md:p-7`}>
              ...
              <h2 className="mt-7 font-display text-2xl font-extrabold">{portal.label}</h2><p className="mt-2 text-sm leading-6 text-[var(--muted)]">{portal.description}</p><p className="mt-6 text-xs font-bold uppercase tracking-[.14em] text-[var(--care)]">Click to continue →</p>
            </button>
          ))}
        </div>
