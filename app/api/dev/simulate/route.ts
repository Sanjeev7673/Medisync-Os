...
export async function POST(req: NextRequest) {
  ...
  const record = getRequest(request_id);
  ...
  switch (record.workflow_status) {
    case "CREATED":
      ...
      appendAudit({ request_id, event_type: "REQUEST_CLASSIFIED", detail: "Classified as SPECIALIST_REVIEW (Cardiology)", actor: "system" });
      break;
    case "PENDING_REVIEW":
      ...
      appendAudit({ request_id, event_type: "SPECIALIST_APPROVED", detail: "Approved by DR-CARD-01", actor: "DR-CARD-01" });
      break;
    case "HOSPITAL_MATCHING":
      ...
      appendAudit({ request_id, event_type: "HOSPITAL_MATCH_GENERATED", detail: "1 recommendation (Hospital A, 94%)", actor: "system" });
      appendAudit({ request_id, event_type: "REFERRAL_CREATED", detail: "REF-2001", actor: "system" });
      break;
    case "REFERRAL_CREATED":
      record.appointment = { status: "PENDING", scheduled_at: null };
      record.workflow_status = "APPOINTMENT_PENDING";
      appendAudit({ request_id, event_type: "NOTIFICATION_SENT", detail: "Patient notified: appointment pending", actor: "system" });
      break;
    case "APPOINTMENT_PENDING":
      ...
      appendAudit({ request_id, event_type: "APPOINTMENT_CREATED", detail: record.appointment.scheduled_at ?? "", actor: "system" });
      break;
    default:
      break;
  }
