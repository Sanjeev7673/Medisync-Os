...
export async function GET(req: NextRequest) {
  ...
  return NextResponse.json({ requests: listRequestsForPatient(patientId) });
}
...
export async function POST(req: NextRequest) {
  ...
  appendAudit({ request_id: record.request_id, event_type: "REQUEST_CREATED", detail: `Request submitted via ${record.request_source}`, actor: record.patient_id });
