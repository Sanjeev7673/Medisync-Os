...
async function assertWorkflowActor(session: Session, row: RequestRow, nextStage: RequestStage) {
  ...
  if (session.role === "specialist") {
    ...
    if (error) throw error;
    if (!data || data.user_id !== session.sub) throw new Error("FORBIDDEN");
...
function toPatientRequest(row: RequestRow): PatientRequest {
  ...
  return {
    request_id: row.request_id, patient_id: row.patient_id, request: row.description, request_source: row.request_source, document_uploaded: row.document_uploaded,
...
export async function createRequest(session: Session, input: { request: string; document_uploaded?: boolean; request_source?: PatientRequest["request_source"] }) {
  ...
  const { data, error } = await getDb().from("requests").insert(row).select("*").single<RequestRow>(); if (error) throw error;
...
export async function assignSpecialist(session: Session, requestId: string, specialistId: string) {
  if (!["admin", "hospital"].includes(session.role)) throw new Error("FORBIDDEN"); const current = await getRequestRow(requestId); if (!current) throw new Error("NOT_FOUND");
  ...
  const { data, error } = await getDb().from("requests").update({ assigned_specialist_id: specialistId }).eq("id", current.id).eq("updated_at", current.updated_at).select("*").single<RequestRow>(); if (error) throw error;
...
export async function advanceRequest(session: Session, requestId: string, workflowStage: RequestStage) {
  const current = await getRequestRow(requestId); if (!current) throw new Error("NOT_FOUND");
  if (!transitions[current.workflow_stage].includes(workflowStage)) throw new Error("INVALID_TRANSITION"); await assertWorkflowActor(session, current, workflowStage);
  const { data, error } = await getDb().from("requests").update({ workflow_stage: workflowStage }).eq("id", current.id).eq("workflow_stage", current.workflow_stage).eq("updated_at", current.updated_at).select("*").single<RequestRow>(); if (error) throw error;
...
export async function updateClassification(session: Session, requestId: string, input: { requestType: RequestType; specialty?: string; specialistReviewRequired?: boolean; documentRequired?: boolean; reason?: string; confidence?: number }) {
  ...
  const { data, error } = await getDb().from("requests").update({ request_type: input.requestType, workflow_stage: "CLASSIFIED", ai_classification, ai_confidence: input.confidence ?? null, ai_reason: input.reason ?? null }).eq("id", current.id).eq("workflow_stage", "CREATED").eq("updated_at", current.updated_at).select("*").single<RequestRow>(); if (error) throw error;
