...
export type RequestType = "GENERAL" | "APPOINTMENT" | "DOCUMENT" | "SPECIALIST_REVIEW" | "HOSPITAL_MATCH" | "ADMINISTRATION";

export type WorkflowStatus = "CREATED" | "VALIDATING" | "CLASSIFIED" | "ROUTED" | "PENDING_REVIEW" | "UNDER_REVIEW" | "APPROVED" | "REJECTED" | "MORE_INFORMATION_REQUIRED" | "HOSPITAL_MATCHING" | "REFERRAL_CREATED" | "APPOINTMENT_PENDING" | "COMPLETED" | "CANCELLED" | "FAILED";

export interface HospitalRecommendation { hospital_id: string; hospital_name: string; match_score: number; matched_capabilities: string[]; missing_capabilities: string[]; reason: string; }
...
export interface PatientRequest {
  ...
  appointment: { status: "PENDING" | "SCHEDULED" | "COMPLETED" | null; scheduled_at: string | null; };
  workflow_status: WorkflowStatus; created_at: string; updated_at: string;
}

export interface AuditEvent { audit_id: string; request_id: string; event_type: string; detail: string; actor: string; created_at: string; }
