...
export default function RoleShell({ role, children }: { role: keyof typeof navByRole; children: ReactNode }) {
  ...
  useEffect(() => {
    fetch("/api/auth/session").then((r) => r.json()).then((data) => setUser(data?.user ?? null)).catch(() => undefined);
  }, []);
    ...
    <div className="mesh-bg min-h-screen text-[var(--ink)]">
      <aside className="fixed inset-y-0 left-0 z-30 hidden w-72 p-5 lg:block">
        <div className="glass flex h-full flex-col rounded-[30px] p-4 shadow-[var(--shadow-md)]">
          ...
          <div className="mt-auto border-t border-black/5 pt-4"><Link href="/api/auth/logout" className="flex items-center justify-between rounded-2xl px-3 py-3 text-sm font-semibold text-[var(--muted-strong)] hover:bg-black/5"><span>Sign out</span><span>↗</span></Link></div>
        </div>
      </aside>
      <main className="min-h-screen lg:pl-72"><div className="mx-auto max-w-7xl px-5 py-5 md:px-8 md:py-8"><header className="mb-8 flex items-center justify-between lg:hidden"><Link href="/dashboard" className="flex items-center gap-2"><Image src="/medisync-mark.svg" alt="MediSync" width={38} height={38} className="rounded-xl" /><span className="font-display font-extrabold">MediSync</span></Link><Link href="/api/auth/logout" className="rounded-xl bg-white/70 px-3 py-2 text-xs font-bold">Sign out</Link></header>{children}</div></main>
    </div>
  );
}
